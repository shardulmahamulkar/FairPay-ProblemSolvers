    ...
    <div className="space-y-6 animate-fade-in pt-4">
      ...
      <Card className="p-5 border-0 shadow-sm rounded-[28px] relative overflow-hidden" style={{ background: "#CFE3E7" }}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-[1.2] flex flex-col justify-center items-center gap-4">
            ...
            <div className="text-center font-medium text-[#1E293B] leading-snug text-[13px]">
      ...
      <div className="grid grid-cols-4 gap-3">
        {speedTiles.map((tile) => (
          ...
          >
            ...
            <span className="text-[13px] text-[#334155] font-semibold text-center leading-tight">{tile.label}</span>
          </button>
        ))}
      </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[19px] font-semibold text-[#0F172A]">Active Groups</h3>
          <button onClick={() => navigate("/groups")} className="text-[15px] text-[#0F172A] hover:text-primary transition-colors flex items-center">
            ...
            >
              ...
              <div className="p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[12px] font-medium text-[#64748B]">{group.members?.length || 0} members</span>
                </div>
              </div>
            </Card>
          ))}
      ...
      {friends.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[19px] font-semibold text-[#0F172A]">Friends</h3>
            <button onClick={() => navigate("/friends")} className="text-[15px] text-[#0F172A] hover:text-primary transition-colors flex items-center">
              See all <ChevronRight className="w-[18px] h-[18px] ml-0.5 text-[#3b82f6]" strokeWidth={2.5} />
            </button>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
            {friends.map((friend) => {
              ...
              return (
                <div key={friend._id} className="flex flex-col items-center gap-1.5 min-w-[64px] flex-shrink-0">
                  ...
                  <span className="text-[13px] text-[#0F172A] font-medium text-center leading-tight w-16 truncate mt-1">{name}</span>
                  {convertedBal !== 0 && (
      ...
      {/* Transactions */}
      <section>
        <Card className="pt-6 pb-4 px-6 rounded-[32px] border-0 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[19px] font-semibold text-[#0F172A]">Transactions</h3>
            <button onClick={() => navigate("/activity")} className="text-[15px] text-[#0F172A] hover:text-primary transition-colors flex items-center">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col justify-center gap-1 mt-0.5">
                      <p className="text-[16px] font-medium text-[#0F172A] leading-none tracking-tight">{exp.expenseNote || "Transaction"}</p>
                      <p className="text-[14px] text-[#64748B] leading-none tracking-tight">{groupName}</p>
                    </div>
                  </div>
                  <p className="text-[16px] font-semibold text-[#334155] tracking-tight">
